...
function onChangeCableCode() {
	var s2 = document.getElementById("cableCode");
	var s3 = document.getElementById("dbNo");
	s3.innerHTML = "";
	let p = fetch("http://ec2-65-0-4-145.ap-south-1.compute.amazonaws.com:8080//CableManagementSystem/DbNumber?cableCode=" + s2.value)
	p.then((value1) => {
		return value1.json()
	}).then((dbNumbers) => {
		var newOption = document.createElement("option");
		...
		for (let i = 0; i < dbNumbers.length; i++) {
			var newOption = document.createElement("option");
			newOption.value = dbNumbers[i];
			newOption.innerHTML = dbNumbers[i];
			s3.options.add(newOption);
		}
	})
}
function onChangeDbNumber() {
	var s2 = document.getElementById("cableCode");
	var s3 = document.getElementById("dbNo");
	var s4 = document.getElementById("drumNo");
	s4.innerHTML = "";
	let p = fetch("http://ec2-65-0-4-145.ap-south-1.compute.amazonaws.com:8080//CableManagementSystem/DrumNumber?cableCode=" + s2.value + "&dbNo=" + s3.value)
	p.then((value1) => {
		return value1.json()
	}).then((drumNumbers) => {
		var newOption = document.createElement("option");
...
function onChangeDrumNumber() {
	var s2 = document.getElementById("cableCode");
	var s3 = document.getElementById("dbNo");
	var s4 = document.getElementById("drumNo");
	var s5 = document.getElementById("length");

	let p = fetch("http://ec2-65-0-4-145.ap-south-1.compute.amazonaws.com:8080//CableManagementSystem/DrumLength?cableCode=" + s2.value + "&dbNo=" + s3.value + "&drumNo=" + s4.value)
	p.then((value1) => {
		return value1.json()
	}).then((length) => {
		console.log("hello");
		console.log(length.drumLength);
		s5.value = length.drumLength;
